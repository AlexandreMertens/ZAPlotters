...
mbbs= [26, 30, 35, 40, 46, 53, 61, 70, 81, 93, 107, 123, 142, 163, 188, 216, 248, 286, 329, 378, 435, 500, 575, 662, 761]
#mbbs= [662]
numDirs=35
  ...
  for i in range(1,numDirs):
    ...
    if mbb < mllbb-10 :
      try:
        ...
        print "mllbb : ", mllbb, "mbb : ", mbb

        print "eff : ", eff, " ratio :", ratio
        ...
        SignalYields = 19.7*eff*ratio #*ggH_*HZA_*Abb_*0.06729
	  ...
	  n+=1
          print "n ", n
	  myTG_2.SetPoint(n,int(mllbb),tree_Sig1.limit/SignalYields)
	  TGAS_1.SetPoint(n,int(mllbb),tree_Sig1.limit/SignalYields)
          TGAS_2.SetPoint(n,int(mllbb),tree_Sig1.limit/SignalYields)
	  ...
	  TGAS_2.SetPointEYlow(n,abs(tree_Sig1.limit/SignalYields-exp_limit))
          ...
          myTG_5.SetPoint(n,int(mllbb),tree_Sig1.limit/SignalYields) 
        
          #tree_SigY.GetEntry(5)	
          #myTG_Y.SetPoint(n,int(mllbb),tree_SigY.limit)
      except:
...
#TH1_4.SetFillColor(kYellow)
  ...
  #myTG_5.SetLineColor(kRed)
  myTG_5.SetLineWidth(3)

  title = "M_{A} = "+str(int(mbb))
  TGAS_2.SetTitle(title)
  TGAS_2.GetXaxis().SetTitle("M_{H}")
  ...
  #f.Open()
  C.Write()
  C.Print("plots_v1/BBP_"+str(int(mbb))+"_Y.pdf","pdf")
  f.Close()
