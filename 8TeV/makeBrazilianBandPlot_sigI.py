...
DataCards_path = "/nfs/user/acaudron/unblindedDatacards2HDMSignalInjec/Combined/"
...
for j in range(len(mllbbs)):
  ...
  MH_dir_SigI = "v1_MH_"+str(int(mllbb))+"_Asymptotic_Unblind_Combined_SigInjected_"
  MH_dir_Sig1 = "v1_MH_"+str(int(mllbb))+"_Asymptotic_Unblind_Combined_"

  if run_combine == 1:
    mkdir_cmd = "mkdir "+MH_dir_SigI
    print mkdir_cmd
    rmdir_cmd = "rm -rf "+MH_dir_SigI
    os.system(rmdir_cmd)
  ...
  for i in range(1,numDirs):
    ...
    file_path_SigI = MH_dir_SigI+"/higgsCombineTest.Asymptotic.mA"+str(int(mbb))+".root"
	...
	if run_combine == 1:
          ...
          cp_cmd = "cp higgsCombineTest.Asymptotic.mH"+str(int(mbb))+".root "+file_path_SigI
	...
	f_SigI=TFile(file_path_SigI)
        tree_SigI = f_SigI.Get("limit")
	  ...
	  tree_SigI.GetEntry(5) 
          myTG_I.SetPoint(n,int(mbb),tree_SigI.limit)

	  print "limit_Sig1", tree_Sig1.limit/SignalYields
	  print "limit_SigY", tree_SigY.limit
	  print "limit_SigI", tree_SigI.limit
  ...
  myTG_I.SetLineColor(kRed)
  myTG_I.SetLineWidth(1)
